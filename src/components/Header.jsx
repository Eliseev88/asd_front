...
function Header() {
  ...
  useEffect(() => {
    ...
    isOpenMenu
      ? (CONTENT.style.top = `-${scrollPos - 75}px`)
      : (CONTENT.style.top = "");
  ...
  return (
    <header className={["header", isOpenMenu ? "__open" : ""].join(" ")}>
      <div className="header__wrp">
        ...
        <Button className="header__btn" role="link" onClick={goToVM}>
          {t("Личный кабинет")}
        </Button>
        ...
        <button
          className={["burger", isOpenMenu ? "__open" : ""].join(" ")}
          onClick={toggleMenu}
        ></button>
      </div>
